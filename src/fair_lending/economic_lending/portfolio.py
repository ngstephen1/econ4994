...
def solve_fixed_request_portfolio(
    ...
    *,
    ...
    constraint = LinearConstraint(principal.reshape(1, -1), lb=-np.inf, ub=float(bank_budget))
    started = perf_counter()
    result = milp(
        c=-profit,
        integrality=np.ones(len(source), dtype=int),
    ...
    if funded.sum() > bank_budget + max(1e-6, abs(bank_budget) * 1e-10):
